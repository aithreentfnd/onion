...
#全新 UI با تم‌های آبی روشن، دارک و مشکی
...
[data-theme="black"]{
  ...
  --accent:#4B8BF7;
  --accent2:#7BABF7;
  --accent-glow:rgba(75,139,247,0.25);
  --text:#FFFFFF;
  --text2:#8899AA;
  --border:rgba(75,139,247,0.15);
}
html,body{height:100%;overflow:hidden}
body{font-family:'Vazirmatn',sans-serif;background:var(--bg-start);color:var(--text);display:flex;align-items:center;justify-content:center;padding:20px;transition:background .4s,color .4s}
.bg{position:fixed;inset:0;background:radial-gradient(ellipse 80% 60% at 50% 0%,var(--accent-glow),transparent 70%),var(--bg-start);z-index:0;transition:background .4s}
...
.o1{width:380px;height:380px;background:var(--accent-glow);top:-100px;right:-80px;opacity:.3}
...
.card{background:var(--card);border:1px solid var(--border);border-radius:24px;padding:38px 34px 34px;backdrop-filter:blur(24px);box-shadow:0 0 80px var(--accent-glow),0 20px 60px rgba(0,0,0,.5)}
.brand{display:flex;align-items:center;gap:14px;margin-bottom:28px}
.brand-img{width:48px;height:48px;border-radius:50%;overflow:hidden;border:2px solid var(--border);box-shadow:0 0 20px var(--accent-glow);flex-shrink:0}
...
h1{font-size:22px;font-weight:700;color:var(--text);margin-bottom:5px;letter-spacing:-.02em}
.sub{font-size:12px;color:var(--text2);margin-bottom:24px;line-height:1.6}
.hint{display:flex;align-items:center;gap:10px;background:rgba(59,130,246,0.07);border:1px solid var(--border);border-radius:12px;padding:10px 14px;margin-bottom:20px}
.hint-label{font-size:11px;color:var(--text2);flex:1}
.hint-val{font-family:ui-monospace,monospace;font-size:14px;font-weight:700;color:var(--accent);background:rgba(59,130,246,0.1);border:1px solid var(--border);padding:3px 11px;border-radius:7px;cursor:pointer;transition:.15s;letter-spacing:.08em}
.hint-val:hover{background:rgba(59,130,246,0.22)}
.field{margin-bottom:18px}
.field label{display:block;font-size:10.5px;font-weight:600;color:var(--text2);margin-bottom:7px;text-transform:uppercase;letter-spacing:.06em}
.inp-wrap{position:relative}
input[type=password]{width:100%;padding:13px 44px 13px 16px;border-radius:12px;border:1px solid var(--border);background:rgba(0,0,0,.3);color:var(--text);font-family:inherit;font-size:14px;outline:none;transition:.2s}
input[type=password]:focus{border-color:var(--accent);background:rgba(0,0,0,.4);box-shadow:0 0 0 3px rgba(59,130,246,.1)}
.ic{position:absolute;left:14px;top:50%;transform:translateY(-50%);color:var(--text2);font-size:18px;pointer-events:none;transition:.2s}
...
.btn{width:100%;padding:13px;border-radius:12px;border:none;cursor:pointer;background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;font-family:inherit;font-size:14px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 20px var(--accent-glow);transition:.2s;position:relative;overflow:hidden}
.btn::before{content:'';position:absolute;inset:0;background:rgba(255,255,255,.08);opacity:0;transition:.2s}
.btn:hover::before{opacity:1}
.btn:disabled{opacity:.5;cursor:not-allowed}
.footer{margin-top:22px;padding-top:18px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:center;gap:8px;font-size:11px;color:var(--text2)}
.footer a{color:var(--accent);font-weight:600;text-decoration:none;display:flex;align-items:center;gap:4px}
.theme-selector{display:flex;gap:8px;margin-top:16px;justify-content:center}
.theme-dot{width:28px;height:28px;border-radius:50%;border:2px solid var(--border);cursor:pointer;transition:.2s;display:flex;align-items:center;justify-content:center;font-size:12px;color:var(--text)}
.theme-dot:hover{transform:scale(1.1)}
.theme-dot.active{border-color:var(--accent);box-shadow:0 0 12px var(--accent-glow)}
...
<div class="wrap">
  <div class="card">
    ...
    <div class="theme-selector">
      <span class="theme-dot active" data-theme="dark" onclick="setTheme('dark')" title="دارک">🌙</span>
      <span class="theme-dot" data-theme="light" onclick="setTheme('light')" title="روشن">☀️</span>
      <span class="theme-dot" data-theme="black" onclick="setTheme('black')" title="مشکی">⬛</span>
    </div>
  </div>
</div>
<script>
function setTheme(t){
  ...
  localStorage.setItem('spacezone-theme',t);
  document.querySelectorAll('.theme-dot').forEach(d=>d.classList.toggle('active',d.dataset.theme===t));
}
const saved=localStorage.getItem('spacezone-theme')||'dark';
setTheme(saved);
...
:root{
  ...
  --card:#0d1b2e;--card-b:rgba(59,130,246,0.15);--card-bh:rgba(59,130,246,0.3);
...
[data-theme="light"]{
  --bg:#E8F2FF;--bg2:#D6E4FF;--bg3:#C4D6F5;
  --card:#FFFFFF;--card-b:rgba(37,99,235,0.15);--card-bh:rgba(37,99,235,0.35);
  --accent:#2563EB;--accent2:#3B82F6;--accent-d:rgba(37,99,235,0.08);
...
[data-theme="black"]{
  ...
  --card:#0d0d0d;--card-b:rgba(75,139,247,0.12);--card-bh:rgba(75,139,247,0.25);
  --accent:#4B8BF7;--accent2:#7BABF7;--accent-d:rgba(75,139,247,0.08);
...
.logo-img{width:38px;height:38px;border-radius:50%;overflow:hidden;border:2px solid var(--card-b);box-shadow:0 0 14px rgba(59,130,246,.25);flex-shrink:0}
...
.mob-logo{width:28px;height:28px;border-radius:50%;overflow:hidden;box-shadow:0 0 8px rgba(59,130,246,.25)}
...
.btn-p{background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;box-shadow:0 2px 14px rgba(59,130,246,.35)}
.btn-p:hover{box-shadow:0 4px 18px rgba(59,130,246,.4)}
...
<section class="pg on" id="pg-overview">
  ...
  <div class="g2">
    <div class="card">
      ...
      <div class="sr"><span class="sr-k"><i class="ti ti-bolt"></i> Siz10a XHTTP Ultra</span><span class="sr-v" style="color:var(--green-t)">● فعال · 2 mode</span></div>
...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب v10.0 — طراحی مدرن با تم‌های جدید"""
...
:root{{
  ...
  --card:#0c1326;--card-b:rgba(59,130,246,0.14);--card-bh:rgba(59,130,246,0.3);
  --accent:#3B82F6;--accent2:#60A5FA;--accent-d:rgba(59,130,246,0.1);
...
[data-theme="black"]{{
  ...
  --red:#EF4444;--red-bg:rgba(239,68,68,0.08);--red-t:#F87171;
  --amber:#F59E0B;--amber-bg:rgba(245,158,11,0.08);--amber-t:#FCD34D;
  --purple:#8B5CF6;--purple-bg:rgba(139,92,246,0.08);--purple-t:#A78BFA;
  --t1:#FFFFFF;--t2:#8899AA;--t3:#556677;
...
.bg-fx{{position:fixed;inset:0;background:radial-gradient(ellipse 70% 45% at 50% -8%,rgba(59,130,246,0.13),transparent 62%),var(--bg);z-index:0;pointer-events:none;transition:background .35s}}
.grid-fx{{position:fixed;inset:0;background-image:linear-gradient(rgba(59,130,246,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,0.025) 1px,transparent 1px);background-size:46px 46px;z-index:0;pointer-events:none}}
...
.brand-img{{width:40px;height:40px;border-radius:50%;overflow:hidden;border:2px solid var(--card-b);box-shadow:0 0 14px rgba(59,130,246,.25);flex-shrink:0}}
.brand-img img{{width:100%;height:100%;object-fit:cover}}
...
.theme-dot-btn{{width:28px;height:28px;border-radius:50%;border:2px solid var(--card-b);cursor:pointer;transition:.18s;display:flex;align-items:center;justify-content:center;font-size:12px;background:var(--card)}}
.theme-dot-btn:hover{{transform:scale(1.1)}}
.theme-dot-btn.active{{border-color:var(--accent);box-shadow:0 0 12px rgba(59,130,246,.3)}}
...
.sub-info::before{{content:'';position:absolute;top:0;right:0;width:160px;height:160px;background:radial-gradient(circle at top right,rgba(59,130,246,.1),transparent 70%);pointer-events:none}}
...
.copy-all-bar{{display:flex;align-items:center;gap:12px;background:linear-gradient(120deg,var(--accent) 0%,#2952C8 100%);border-radius:18px;padding:16px 19px;margin-bottom:18px;box-shadow:0 10px 30px rgba(59,130,246,.28);flex-wrap:wrap}}
...
.ubar{{height:6px;border-radius:4px;background:rgba(59,130,246,0.1);overflow:hidden;margin-bottom:5px}}
...
[data-theme="light"] .cfg-vless{{background:rgba(37,99,235,.05)}}
...
.btn i{{font-size:13px}}
.btn-p{{background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;box-shadow:0 3px 14px rgba(59,130,246,.35)}}
.btn-p:hover{{background:var(--accent2)}}
.btn-g{{background:var(--accent-d);color:var(--accent2);border:1px solid rgba(59,130,246,.16)}}
.btn-g:hover{{background:rgba(59,130,246,.2)}}
...
.lock-banner{{background:linear-gradient(150deg,rgba(59,130,246,.16),rgba(59,130,246,.02) 70%);padding:38px 30px 26px;position:relative}}
...
[data-theme="light"] .lock-inp{{background:rgba(37,99,235,.04)}}
...
<div class="wrap">
  <div class="top">
    ...
    <div class="top-actions">
      ...
      <span class="theme-dot-btn active" data-theme="dark" onclick="setTheme('dark')" title="دارک">🌙</span>
      <span class="theme-dot-btn" data-theme="light" onclick="setTheme('light')" title="روشن">☀️</span>
      <span class="theme-dot-btn" data-theme="black" onclick="setTheme('black')" title="مشکی">⬛</span>
    </div>
  </div>
  <div id="root">
...
function setTheme(t){
  document.documentElement.setAttribute('data-theme',t);
  localStorage.setItem('spacezone-pub-theme',t);
  document.querySelectorAll('.theme-dot-btn').forEach(d=>d.classList.toggle('active',d.dataset.theme===t));
}
const savedTheme=localStorage.getItem('spacezone-pub-theme')||'dark';
setTheme(savedTheme);

function toggleTheme(){
  const current=document.documentElement.getAttribute('data-theme')||'dark';
  const next=current==='dark'?'light':current==='light'?'black':'dark';
  setTheme(next);
}
